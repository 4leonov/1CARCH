...
Процедура ПолучениеСообщений() Экспорт
	...
	Попытка
		...
		Потребитель = Компонента.BasicConsume(
			НастройкаПодключения.ИмяОчереди, 
			"", 
			Истина, 
			Ложь, 
			0);
			ВходящееСообщениеJSON = "";
	                                                                   
			
			Если Компонента.BasicConsumeMessage("", ВходящееСообщениеJSON, 5) Тогда
				
				Компонента.BasicAck();
				СоздатьКомандировкуНаОсновеЗаявки(ВходящееСообщениеJSON);
...
Процедура СоздатьКомандировкуНаОсновеЗаявки(ЗаявкаJSON) Экспорт
	
	ЧтениеJSON = Новый ЧтениеJSON;
	ЧтениеJSON.УстановитьСтроку(ЗаявкаJSON);
	ЗаявкаСтруктура = ПрочитатьJSON(ЧтениеJSON,,,ФорматДатыJSON.ISO);
	ЧтениеJSON.Закрыть();  
	
	мДатаДокумента 	= ПрочитатьДатуJSON(ЗаявкаСтруктура.Дата, ФорматДатыJSON.ISO);
	мДокумент 		= Документы.Командировка.НайтиПоНомеру(СокрЛП(ЗаявкаСтруктура.Номер), мДатаДокумента);
	мСотрудник  	= Справочники.Сотрудники.НайтиПоКоду(СокрЛП(ЗаявкаСтруктура.Табельный));  
	мПроект  		= Справочники.Проекты.НайтиПоКоду(СокрЛП(ЗаявкаСтруктура.КодПроекта));
	
	Если ЗначениеЗаполнено(мДокумент.Ссылка) Тогда
		
		КомандировкаОбъект = мДокумент.ПолучитьОбъект();
	...
	КомандировкаОбъект.Номер 			= ЗаявкаСтруктура.Номер;
	КомандировкаОбъект.Дата 			= мДатаДокумента;
	КомандировкаОбъект.Сотрудник  		= мСотрудник;    
	КомандировкаОбъект.ФизическоеЛицо 	= КомандировкаОбъект.Сотрудник.ФизическоеЛицо; 
	КомандировкаОбъект.Организация 		= КомандировкаОбъект.Сотрудник.ГоловнаяОрганизация;
	КомандировкаОбъект.ДатаНачала  		= ПрочитатьДатуJSON(ЗаявкаСтруктура.ДатаНачала, ФорматДатыJSON.ISO);
	КомандировкаОбъект.ДатаОкончания 	= ПрочитатьДатуJSON(ЗаявкаСтруктура.ДатаОкончания, ФорматДатыJSON.ISO);
	КомандировкаОбъект.OTUS_Проекты     = мПроект;
	КомандировкаОбъект.Цель             = ЗаявкаСтруктура.Цель;
	КомандировкаОбъект.ПериодРегистрации= НачалоМесяца(ТекущаяДата());  
	КомандировкаОбъект.Комментарий 		= "HRM";
	
	КомандировкаОбъект.Записать(РежимЗаписиДокумента.Запись);
